...
| **daytrade-it**<br/><img src="https://img.shields.io/badge/PRIVATE-64748B?style=flat-square&labelColor=1E293B" alt="PRIVATE"/> | 국내주식(코스피·코스닥) 데이트레이딩 시스템이자 `trader` 의 두 번째 라이브 프로세스. 데몬이 토스 뉴스를 폴링하면 Claude 는 **사실만** 뽑는다 — 재료 유형, 지속성, 방향. 매매 판단은 시키지 않는다. 규칙 기반 가격 확인이 같은 방향일 때만 신호로 남긴다. 처음엔 가격만 보는 ML 모델을 붙여 봤는데 정확도 38.5% 로 다수 클래스 기준선(38.1%)과 차이가 없어 버렸다. **데몬은 주문을 내지 않는다.** 주문은 `execute_trade` 를 명시적으로 불러야 나가고, 거기엔 1주·지정가 전용·가격 밴드·킬스위치·보유 종목 중복 진입 거부가 걸려 있다. DART 중대 공시 리스크 게이트는 만들어 적대적 테스트까지 마쳤지만 아직 그 주문 경로에 공시가 들어가지 않는다. 미국 시장 뉴스-감성 실험이던 `gpt-quant-v2` 를 국내주식용으로 다시 짰다 |
...
| **도는 것** | `scalp-it` 틱·호가 수집과 실주문 · `daytrade-it` 신호 데몬 · `quantbox`(중단 중) · `kiwoom-client` 개발 — 브로커 세션은 키 하나당 하나라 그 세션이 여기 있다 | Airflow 스케줄러·웹서버(16개 DAG) · TimescaleDB **PRIMARY** · `kr-quant`·`portfolio-research`·`macro-sector-agent`·`momentum` · 마감 후 리서치 배치 |
| **공유 레포** | `quant-airflow` 와 `kr-quant` 는 **`git sparse-checkout`** 으로만 있다 — 리플리카 compose, 스키마, 그리고 라이브 프로세스가 브로커 키와 DB 접속 정보를 읽어 가는 `.env` | 정본 전체 클론 |
...
flowchart LR
    ...
    subgraph DTP ["daytrade-it — 신호는 무인, 주문은 아니다"]
        TOSS["토스 뉴스"] --> CL["Claude<br/>사실만 추출, 매매 판단 금지"]
        CL --> TC["규칙 기반 가격 확인<br/>전일 종가 대비"]
        TC -->|"둘이 같은 방향"| SIG[("gptquant<br/>trading_signals")]
        SIG -. "명시적 호출 — 스케줄 없음" .-> ET["execute_trade<br/>1주 · 지정가 · 가격 밴드<br/>live_STOP"]
    end
...
<sub>빨강은 주문 직전의 마지막 관문이다. 점선은 사람이나 에이전트가 직접 해야 하는 단계다.</sub>
...
| **08:55** | 런처 둘이 뜬다 — scalp-it 감지기는 PRIMARY 에서 오늘 유니버스를 읽고, daytrade-it 데몬은 뉴스 폴링을 시작한다 | |
| **09:00–15:20** | scalp-it 수집·매매 · daytrade-it 신호 적재 · 10분마다 DART · 09:10·10:00 틱 헬스체크 | 10:05 `daily_news` 와 수집 캐치업 · 11:35 Airflow 헬스체크 |
...
| **16:55–19:00** | | 수정주가 · 컨센서스 · Sharadar(화–토) · `kr-quant` 일일 리포트 · 커버리지 · 구글 드라이브 백업 |
...
- **라이브 머신은 매일 아침 리서치 머신에 기댄다.** 스풀은 나가는 틱을 지킬 뿐 들어오는 입력은 못 지킨다. scalp-it 의 유니버스와 시황, daytrade-it 의 가격 기준은 전부 PRIMARY 에서 읽는다 — 전날 16:00 수집으로 만든 값이다. 그 조회가 실패하면 scalp-it 은 고정 쌍으로 폴백하고, daytrade-it 은 신호를 남기지 않는다.
- **계좌는 하나인데 킬스위치는 따로다.** 둘 다 매수 직전에 브로커에서 잔고를 조회해 이미 보유한 종목은 사지 않는다. 그래서 먼저 들어간 쪽이 청산할 때까지 그 종목을 가진다. 하지만 `pair_STOP` 은 scalp-it 만, `live_STOP` 은 daytrade-it 만 멈춘다 — 하나를 세워도 다른 하나는 계속 매매할 수 있다.
